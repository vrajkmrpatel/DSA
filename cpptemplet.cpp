...
vi sieve(int n) {int*arr = new int[n + 1](); vi vect; forle(i,2,n)if (arr[i] == 0) {vect.pb(i); for(int j = 2 * i; j <= n; j += i)arr[j] = 1;} return vect;}
int mod_add(int a, int b, int m) {a = a % m; b = b % m; return (((a + b) % m) + m) % m;}
int mod_mul(int a, int b, int m) {a = a % m; b = b % m; return (((a * b) % m) + m) % m;}
int mod_sub(int a, int b, int m) {a = a % m; b = b % m; return (((a - b) % m) + m) % m;}
int mod_div(int a, int b, int m) {a = a % m; b = b % m; return (mod_mul(a, mminvprime(b, m), m) + m) % m;}  //only for prime m
int phin(int n) {int number = n; if (n % 2 == 0) {number /= 2; while (n % 2 == 0) n /= 2;} for (int i = 3; i <= sqrt(n); i += 2) {if (n % i == 0) {while (n % i == 0)n /= i; number = (number / i * (i - 1));}} if (n > 1)number = (number / n * (n - 1)) ; return number;} //O(sqrt(N))
auto find_next_smaller(vector<int> v, const int x) { std::sort(v.begin(), v.end());auto it = std::lower_bound(v.begin(), v.end(), x); if (it == v.end()) { it = (v.rbegin()+1).base();}else if (it != v.begin() && *it > x) { --it; }return *it;}
void leftRotate(int arr[], int d, int n){d = d % n;int g_c_d; gcd(d, n);fori(i,0,g_c_d) {int temp = arr[i];int j = i;while (1) {int k = j + d;if (k >= n)k = k - n;if (k == i)break;arr[j] = arr[k];j = k;}arr[j] = temp;}}
